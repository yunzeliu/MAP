...
from .rope import *
import random
from .pointtransformer import TransformerDecoder
from .pointtransformer import Block as Block_vit
try:
...
class LNPBlock(nn.Module):
    ...
    def forward(self, center, feat):
        ...
        pretrain_flag = 0
        feat = feat[:,1:,:] # B G C
        if center.shape[1] != feat.shape[1]:
...
class Trans3DBlock(nn.Module):
    def __init__(self,
        ...
        self.mixer = Block_vit(dim=dim, num_heads=6, mlp_ratio=4.0, qkv_bias=False, drop=0, attn_drop=0, drop_path=0)
...
class Mamba3DEncoder(nn.Module):
    def __init__(self, k_group_size=8, embed_dim=768, depth=4, drop_path_rate=0., num_group=128, num_heads=6, bimamba_type="v2",):
        ...
        self.blocks_vit = nn.ModuleList([
            Trans3DBlock(dim=embed_dim, #
                k_group_size = self.k_group_size,
                drop_path = drop_path_rate[i] if isinstance(drop_path_rate, list) else drop_path_rate, #
                num_group=self.num_group,
                num_heads=self.num_heads,
    ...
    def forward(self, center, x, pos):
        ...
        for n, block in enumerate(self.blocks):
              ...
              if (n + 1) % 3 == 0:
                  block_idx = n // 3
                  if block_idx < len(self.blocks_vit):
                          x = self.blocks_vit[block_idx](center, x + pos)
