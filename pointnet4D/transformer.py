...
def get_decoder_mask(seq_len):
    """ Generate a square mask for the sequence. The masked positions are filled with False.
        Unmasked positions are filled with True. This mask ensures that each position in the
        sequence can only attend to previous or current positions.
    """
    nopeak_mask = (1 - torch.triu(torch.ones((seq_len, seq_len)), diagonal=1)).bool()
    return nopeak_mask
